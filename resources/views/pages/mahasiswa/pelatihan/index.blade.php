        ...
        <form action="{{ route('pelatihan.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group row">
                <div class="col-lg-4">
                    ...
                    <input type="text" class="form-control @error('durasi') is-invalid @enderror" id="durasi" name="durasi" placeholder="Masukkan Durasi" value="{{ old('durasi') }}">
                    @error('durasi')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
...
<div class="card mb-5">
    <div class="card-body">
        <h3 class="text-center mb-2">Pelatihan</h3>
        <div class="table-responsive text-nowrap">
            <table class="table table-bordered">
                ...
                <tbody>
                    @forelse ($items as $pelatihan)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $pelatihan->penyelenggara }}</td>
                            <td>{{ $pelatihan->tanggal }}</td>
                            <td>{{ $pelatihan->durasi }}</td>
                            <td>{{ $pelatihan->region }}</td>
                            <td>
                                <button type="button" class="btn btn-sm btn-info" data-toggle="modal" data-target="#gambarModal{{ $pelatihan->id }}">
                                    Lihat Sertifikat
                                </button>
                            </td>
                            <td>
                                <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalEdit{{ $pelatihan->id }}">
                                    Edit
                                </button>
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('pelatihan.update', $item2->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="penyelenggara">Penyelenggara</label>
                            ...
                            <div class="form-group row">
                                <div class="col-lg-4">
                                    <label for="durasi">Durasi</label>
                                    <input type="text" class="form-control @error('durasi') is-invalid @enderror" id="durasi" name="durasi" placeholder="Masukkan Durasi" value="{{ $item2->durasi }}">
                                    @error('durasi')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        <img src="{{ asset('storage/assets/file-sertifikat-pelatihan/' . $item3->file_sertifikat) }}" alt="" style="width: 800px">
                    </div>
                </div>
            </div>
        </div>
